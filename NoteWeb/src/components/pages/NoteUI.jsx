...
function NoteUI() {
  ...
  const handleDeleteNote = async (noteId) => {
    try {
      ...
      setNoteToDay((prevNotes) => prevNotes.filter((note) => note.id !== noteId))
  ...
  const handleUpdateNote = async (noteId) => {
    try {
      ...
      setNoteData((prevNotes) => prevNotes.map((note) => (note.id === noteId ? response.data.note : note)))
      setNoteToDay((prevNotes) => prevNotes.map((note) => (note.id === noteId ? response.data.note : note)))
      setIsEditing(false)
      setEditNote(null)
    } catch (error) {
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-7 gap-2 mt-4">
                  {[
                    ...
                    return (
                      ...
                      >
                        <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">{day + 1}</div>
                        <div
...
// Wrap the NoteUI component with ThemeProvider
function NoteUIWithTheme() {
  return (
    <ThemeProvider>
      <NoteUI />
    </ThemeProvider>
...
export default NoteUIWithTheme
